...
+ (SEGAnalyticsConfiguration*)createConfigFromFile {
    ...
    BOOL isBranchIoIntegrationEnabled = [[dict objectForKey: @"com.claimsforce.segment.ENABLE_BRANCHIO_INTEGRATION"] boolValue];
    if(!writeKey) {
        return nil;
    }
...
+ (SEGAnalyticsConfiguration*)createConfigFromDict:(NSDictionary*) dict {
    ...
    BOOL isBranchIoIntegrationEnabled = [[dict objectForKey: @"branchIoIntegrationEnabled"] boolValue];
    SEGAnalyticsConfiguration *configuration = [SEGAnalyticsConfiguration configurationWithWriteKey:writeKey];
    configuration.trackApplicationLifecycleEvents = trackApplicationLifecycleEvents;
